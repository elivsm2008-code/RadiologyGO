...
export default function HandProjectionScreen() {
  const { proyeccion } = useLocalSearchParams<{ proyeccion?: string }>();
  const projection = getHandProjection(proyeccion);
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.header}>
          <Text style={styles.eyebrow}>MANO</Text>
          <Text style={styles.title}>{projection.title}</Text>
          <Text style={styles.subtitle}>{getMasteryStatus(projectionProgress?.mastery ?? 0)} · {projectionProgress?.mastery ?? 0}% de dominio</Text>
          <View style={styles.progress}><MasteryBar value={projectionProgress?.mastery ?? 0} /></View>
        </View>
